...
from law_exporter import export_csv, export_detailed_csv
...
class TestURLAnalyzerLawDetection:
    """URL 分析器法規網站偵測測試"""
    ...
    def test_moj_law_detection(self):
        ...
        with patch.object(analyzer.session, 'head') as mock_head:
            ...
            assert result['strategy'] == 'law_moj'
            assert result['confidence'] == 1.0
            assert any('law' in r.lower() for r in result['reasons'])
    ...
    def test_arkiteki_term_detection(self):
        ...
        with patch.object(analyzer.session, 'head') as mock_head:
            ...
            assert result['strategy'] == 'law_arkiteki'
            assert result['confidence'] == 1.0
