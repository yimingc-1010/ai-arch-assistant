...
class URLAnalyzer:
    ...
    def analyze(self, url: str) -> Dict[str, Any]:
        ...
        Returns:
            Dict containing:
            - strategy: 'api', 'html', 'law_moj', or 'law_arkiteki'
        ...
        # 優先檢查是否為法規網站
        law_strategy = self._check_law_site(url, parsed, result)
        if law_strategy:
            result['strategy'] = law_strategy
    ...
    def _check_law_site(self, url: str, parsed, result: Dict) -> str:
        """檢查是否為法規網站"""
        domain = parsed.netloc.lower()

        for site_domain, strategy in self.LAW_SITES.items():
            if site_domain in domain:
                # 對 arkiteki.com 額外檢查是否為 /term/ 路徑
                if site_domain == 'arkiteki.com':
                    if '/term/' in parsed.path:
                        result['reasons'].append(f'Law site detected: {site_domain} (term page)')
                        return strategy
                else:
                    result['reasons'].append(f'Law site detected: {site_domain}')
                    return strategy
